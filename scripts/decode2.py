...
class Decoder():
    ...
    def decode(self, likelihood, lengths, trans = None):
        ...
        os.system('grep WER %s/wer_* | utils/best_wer.sh' % self.decode_dir)
...
if __name__ == '__main__':
    ...
    decoder = Decoder('data/tree_sp0.5/graph_5gram', 'exp/test/posterior', 'exp/test/uns_decode_5gram_sp0.5')
    '''
    self_p = 0.9
    n_gram = 9
    typ = 'uns'

    likelihood = pkl.load(open('exp/uns_non_matched/phn_prob','rb'))
    graph_dir = 'data/tree_sp{}/graph_{}gram'.format(self_p,n_gram)
    decode_dir = 'exp/uns_non_matched/{}_decode_{}gram_sp{}'.format(typ,n_gram,self_p)
    decoder = Decoder(graph_dir, 'exp/uns_non_matched/posterior', decode_dir)
